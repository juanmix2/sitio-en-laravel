  ...
  <div class="row">
    ...
    <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
      <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">Administrar clase</h1>
      ...
      <div class="my-4 w-100" id="myChart" width="900" height="380">
       ...
       <div class="row">
        <a href="{{ url('/agregarcurso') }}" class="btn btn-outline-success col-2">
         ...
         <table class="table table-striped">
          <thead>
            <tr>
              ...
              <th>Precio</th>
              <th>Clave Clase</th>
              <!--hacer que se desplegue un menu con las opciones de instructor-->
              <th>Clave Instructor</th>
              <!--fin menu con las opciones de instructor-->

              </tr>
              </thead>
               <tbody>
               @foreach($courses as $z)
               <tr>
              <td>{{$z->nombre}}</td>
              <td>{{$z->descripcion}}</td>
              <td>{{$z->lapso}}</td>
              <td>{{$z->precio}}</td>
              <td>{{$z->id}}</td>
              <td>{{$z->instructorfk}}</td>
             
    <td>
              <div class="btn-group">
                <a href="{{url('editarcourse/'.$z->id.'/edit')}}" class="btn btn-outline-success">
                  Editar</a>
                  <form method="post" action="{{ url('/borrarcourse/'.$z->id) }}">
    {{csrf_field()}}
    {{method_field('DELETE')}}
